...
class TurtleGame:
    ...
    bef_time = 0 #ゲームの時間に関するメンバ変数
    count = 0 #時間更新時におけるループ回数を保持するメンバ変数
    ...
    def on_paint(self):
        ...
        etime = now - self.stime
        #もし黒以外のターゲットが１匹以上いたら実行する
        if len(self.targets) - self.black_count > 0:
            self.count += 1
            #5回のループにつき一回表示している秒数を更新
            if self.count % 5 == 0:
                sec = etime.seconds + etime.microseconds / 1000000
                self.ttext.clear()
                self.ttext.write(f"経過時間:{sec:.1f}秒",
                                font = ("helvetica",24))
                #5秒に一回ターゲットを追加する
                if (etime.seconds % 5 == 0) and (self.bef_time != etime.seconds):
                    target = Target()
                    self.targets.append(target)
                    print("Add")
                    self.bef_time = etime.seconds
                    self.rtext.clear()
                    self.rtext.write(f"残りのターゲット:{len(self.targets)}",
                                    font = ("helvetica",24))
        #もし黒色以外のターゲットが0匹だった時の終了するための処理
        else:
            self.ttext.goto(-250, 0)
            self.ttext.write("ミッション完了！", font = ("helvetica",60))
            self.screen.update()
            return
        ...
        if self.is_fallen():
            ...
            for i in range(10):
                ...
                self.player.left(15)
            self.ttext.goto(-280, 0)
            self.ttext.write("ゲームオーバー", font = ("helvetica", 60))
        #もし落ちていなかったら実行
        else:
            #スクリーンの更新処理
            self.screen.update()
            #10ミリ秒に一回このon_paintメソッドを呼び出す
            self.screen.ontimer(self.on_paint, 10)
    ...
    #playerが奈落に落ちたかどうかとtargetが異常に多くいる場合の判定
    def is_fallen(self):
        return self.player.ycor() < -Y_LIMIT or len(self.targets) >= 50
